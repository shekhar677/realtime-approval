    ...
    <div>
      ...
      <div className="p-4 min-h-screen flex justify-center items-center">
        <div className="m-2 bg-white rounded-md border p-4 shadow-md w-5/12">
          ...
          { formData.length? formData.map((form, i) => 
            <div key={i} onClick={ openDialog } className="m-3 cursor-pointer hover:bg-green-200 bg-white rounded-md border p-4 shadow-md flex items-center">
      ...
      <div onClick={closeDialog} className="bg-red-600 p-4 fixed inset-0 flex h-screen justify-center items-center" style={{ display: modal? "flex" : "none", background: 'rgba(0,0,0,0.7)' }}>
        <div className="bg-white rounded-md border p-4 px-6">
          ...
          <p className="text-gray-700 text-xl font-semibold">Do you want to Approve or Reject this request</p>
          <div className="my-3 flex justify-center">
            <button onClick={approveRequest} className="bg-green-300 px-4 py-2 rounded-md text-gray-700 mr-3 hover:bg-green-400 hover:text-white">Approve</button>
            <button onClick={rejectRequest} className="bg-red-300 px-4 py-2 rounded-md text-gray-700 hover:bg-red-400 hover:text-white">Reject</button>
          </div>
        </div>
      </div>
    </div>
